...
func (s *Server) handleStreamProxy(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	...
	if streamSource == nil {
		logging.Error(ctx, nil, "failed to create stream source")
		http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
		return
	}
	...
	if !s.streamManager.HasActiveStream(streamKey) {
		if !s.acquireSemaphores(ctx) {
			logging.Error(ctx, nil, "failed to acquire semaphores")
	...
	reader, err := s.streamManager.ServeStream(streamReq)
	if err != nil {
	...
	if written == 0 {
		logging.Error(ctx, nil, "no data written to response")
